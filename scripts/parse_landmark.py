...
for item_idx, item in enumerate(json_data.values()):
  ...
  # parse landmarks
  lm = numpy.array(item['image']['face_landmarks'])
  lm = lm * scale
  ...
  # eye_left
  mean_left_eye = numpy.mean(lm[map_left_eye], 0)  # (x, y)
  half_len_left_eye = numpy.max((numpy.max(numpy.max(lm[map_left_eye], 0) - numpy.min(lm[map_left_eye], 0)) / 2, 16))
  ...
  loc_left_eye = numpy.hstack((mean_left_eye - half_len_left_eye + 1, mean_left_eye + half_len_left_eye)).astype(int)
  if save_img:
  ...
  # eye_right
  mean_right_eye = numpy.mean(lm[map_right_eye], 0)
  half_len_right_eye = numpy.max((numpy.max(numpy.max(lm[map_right_eye], 0) - numpy.min(lm[map_right_eye], 0)) / 2, 16))
  ...
  # mouth
  mean_mouth = numpy.mean(lm[map_mouth], 0)
  half_len_mouth = numpy.max((numpy.max(numpy.max(lm[map_mouth], 0) - numpy.min(lm[map_mouth], 0)) / 2, 16))
  item_dict['mouth'] = [mean_mouth[0], mean_mouth[1], half_len_mouth]
  # mean_mouth[0] = 512 - mean_mouth[0]  # for testing flip
  loc_mouth = numpy.hstack((mean_mouth - half_len_mouth + 1, mean_mouth + half_len_mouth)).astype(int)
  if save_img:
